...
function ActivePolls() {
  ...
  const fetchActivePolls = async () => {
    ...
    try {
      ...
      const filteredPolls = activePolls.filter(poll => {
          const isResponded = respondedPollIds.has(poll.id);
          ...
          return true; // Is currently active and available
      });
  ...
  const handleSubmitPoll = async (poll) => {
    ...
    try {
        const pointsAwarded = poll.rewardPoints !== undefined ? poll.rewardPoints : 10;
        ...
        await runTransaction(db, async (transaction) => {
            ...
            const newPoints = (userDoc.data().pointsEarned || 0) + pointsAwarded;
            ...
            transaction.set(transactionRef, {
                ...
                description: `Reward for completing poll: "${poll.question}"`,
      ...
      {polls.map(poll => {
          ...
          return (
            <div key={poll.id} style={{ border: '1px solid #eee', borderRadius: '8px', padding: '1.5rem', marginBottom: '1.5rem' }}>
              ...
              <p style={{ fontSize: '0.9rem', color: '#4CAF50', fontWeight: 'bold', margin: '1.5rem 0' }}>Reward: {pointsAwarded} Points</p>
              <button 
